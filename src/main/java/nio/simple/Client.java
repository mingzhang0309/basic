...
public class Client {
    ...
    public String write(String toSend) {
        ...
        /*
         * System.out.println("STARTING_WRITE " + " " + new String(buffer.array(), 0, buffer.limit()));
         */
        selector.wakeup();
            ...
            while (true) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (key.isConnectable()) {
                        ...
                        if ((read < 0 && buffer.position() > 0) || buffer.position() == contentLengh) {
                            if (keyChannel.isConnected()) {
                                System.out.println("READ COMPLETE");
                                buffer.flip();
                                result = new String(buffer.array(), 0, buffer.limit());
                                contentLengh = null;
                                buffer = null;
                                key.interestOps(SelectionKey.OP_WRITE);
                                iterator.remove();
                        ...
                        if (buffer.remaining() == 0) {
                            ...
                            System.out.println("WRITE COMPLETE " + key.interestOps());
                            iterator.remove();
                        }
                    }
                }
    ...
    private void printKeyStates(Set<SelectionKey> keys) {
        if (keys != null)
            for (SelectionKey key : keys)
                if (key.isValid())
                    System.out.println("KEY=" + key.isAcceptable() + " " + key.isConnectable() + " " + key.isReadable()
                            + " " + key.isWritable());
        System.out.println("----");
    }
}