...
public class Server {
    ...
    static Map<SelectionKey, Integer> contentLenths = Collections.synchronizedMap(new HashMap<SelectionKey, Integer>());
            ...
            while (iterator.hasNext()) {
                ...
                if (key.isAcceptable()) {
                    ...
                    if (buffer.remaining() == 0) {
                        ByteBuffer buffer1 = ByteBuffer.allocate(buffer.capacity() + 1024);
                    ...
                    try {
                        ...
                        if (contentLengh == null && read > 0) {
                            ...
                            for (i = 0; i < b.length && i < MAX_HEADER_LOCATION; i++) {
                                if (b[i] == NEXT_LINE) {
                                    ...
                                    contentLengh = Integer.parseInt(new String(b, 0, i));
                                    contentLenths.put(key, contentLengh);
                                    System.out.println("HEADER_FOUND=" + contentLengh + " " + i + " " + b.length);
                                    break;
                                }
                            }
                            if (contentLengh != null) {
                                ...
                                System.out.println("HEEAADDEERR =" + buffer.position() + " " + i);
                                int pos = buffer.position();
                                if (pos > i + 1) {
                                    System.arraycopy(b, i + 1, b1, 0, b.length - i - 1);
                                    System.out.println("COPIED='" + new String(b1) + "'");
                        ...
                        System.out.println("READ " + read + " " + buffer.position() + " " + buffer.limit() + " "
                                + new String(buffer.array(), 0, buffer.position()));
                        if (buffer != null && contentLengh != null
                                && ((read < 0 && buffer.position() > 0) || buffer.position() == contentLengh)) {
                    ...
                    SocketChannel channel = (SocketChannel) key.channel();
                    /*
                     * if (buffer.position() == 0) { String header = buffer.limit() + "\n";
                     * System.out.println("SENDING CONTENT LENGTH  " + "'" + header + "'");
